"""一般ノイズ軽減の適用層のテスト(mocapvmd.md §4.2 / 実装計画 §4.1, §5.5)。

apply_denoise は密サンプル(位置・回転)とクリーニングパラメータ(窓・強度)から、平滑化済みの
密サンプルを返す。位置は中央値フィルタ後 Savitzky-Golay を位置強度でブレンド、回転は窓内正規化平均を
回転強度でブレンド。スパイクは抑制し、アクセント・カット境界・範囲端は保護(変更しない)。1フレーム
あたりの元値からの変更は 位置 各軸 0.3 / 回転 5度 にクランプする(§5.5)。
