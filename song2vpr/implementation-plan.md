...
**主たる新規リスクはピッチ系**(歌唱の F0 は装飾・ずれが多く、音符化が難しい)。音素認識は `vocal_analysis` の
S-1ゲート合格を前提とする既知リスク。ピッチ系は各ステップで次の手順・評価観点で確定し de-risk する(根拠の無い
具体ライブラリは事前に断定しない)。

- **F0/ピッチ推定の手法(T-2 で確定)**: T-2 の冒頭に候補(F0推定アルゴリズム・ライブラリ)の比較スパイクを置き、
  代表歌唱サンプルで**評価観点**(有声/無声判定の正確さ、F0 の安定性、半音への量子化誤差、ビブラート/
  グリッサンドでの破綻の少なさ)を測って採用を決める。採用手法・有声/無声判定・F0→音高(半音)写像と量子化規則は
  T-2 で固定する。
- **音符分割規則(T-3 で確定)**: F0 の安定区間・音素境界・無音から音符を切る規則、同一音高の連結、短い音符の
  扱いを T-3 で確定する。評価観点は参照(手譜またはペアの vpr)に対する音符の過不足・境界ずれ。
- **テンポ・拍子(T-3/T-4 で確定)**: テンポの推定可否(`--tempo` 指定が無い場合)・拍子の扱い・秒⇔tick/拍 の
  変換規則を確定する。評価観点は BPM 誤差・拍位置ずれ・指定テンポ優先時の変換誤差。自動推定の信頼度が低い
  場合は `--tempo` 指定を促す(自動推定を諦める条件)。
- **vpr 形式の確定**: `vpr_io` の write 拡張(vpr レイアウト確定)が T-4 の前提になる
  ([vpr_io 実装計画](../vpr_io/implementation-plan.md) V-3)。
- 上記の初期値・採用基準は代表サンプルで調整し、実音声での F0 精度・音符化精度の確認は重テストとして分離する
  (§3)。
