...
MMD で開ける妥当な VMD を生成する（§4）。生成した JSON は閲覧・手編集・差分確認にも使える。
...
|---|---|---|
| `header` | object | §2.1。 |
| `bone` | array | §3.1。ボーンキー列。空配列可。 |
| `morph` | array | §3.2。モーフキー列。空配列可。 |
| `camera` | array | §3.3。カメラキー列。空配列可。 |
| `light` | array | §3.4。照明キー列。空配列可。 |
| `self_shadow` | array | §3.5。セルフ影キー列。空配列可。 |
| `ik_property` | array | §3.6。IK・プロパティキー列。空配列可。 |
| `sections` | object | §2.2。旧版 VMD で省略された後方セクションの有無。 |
