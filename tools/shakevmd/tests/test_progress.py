...
def test_format_line_hides_count_when_total_unset():
    ...
    assert progress._format_line("ベイク", 0, None, 2.0) == "[ベイク] 経過 0:02"
...
def test_update_note_appears_in_drawn_line():
    # update の note が描画行に併記される(出力後検証の表示確認)。stage() は note をリセットする。
    stream = io.StringIO()
    clock = [0.0]
    r = ProgressReporter(stream, enabled=True, now=lambda: clock[0], interval=3600.0)
    r.stage("スムージング")
    ...
    r.stage("ベイク")
    r._draw()
    assert "出力後検証" not in stream.getvalue().rsplit("\r", 1)[1]
    r.close()
