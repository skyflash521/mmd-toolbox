...
VOCALOID プロジェクトファイル(vpr)から、標準口モーフ(あ・い・う・え・お・ん)によるリップモーションVMDを
生成する独立CLIツールの要求仕様。
...
入力 vpr
  ...
  └─ モーフ生成             lipsync(共有)+ vmd → あ・い・う・え・お・ん のリップモーションVMD
```
...
- **口形イベント列の確定(vpr2vmd 入口)**: `vpr` が返す音符の音素から母音(a/i/u/e/o)・両唇閉鎖・撥音(ん)を、
  ...
  時間変換に用いない)。確定した口形イベント列(母音・両唇閉鎖・撥音「ん」・無音・レガート間隙)を `lipsync` の入力とする。リップモーションは単一の
