...
- **音符内の時間配分**(vpr2vmd 固有): 音符区間から語頭両唇閉鎖の取り分 `d_b` を引いた残り
  `[音符始端 + d_b, 音符終端)` を、その音符の母音の個数で均等分割する。末尾の母音の終端だけは等分割の
  計算値でなく音符終端をそのまま使い、分割の丸め誤差を音符境界へ持ち込まない。`d_b` は、語頭が両唇音なら
  公称長(初期3フレーム)と音符長に対する取り分上限(初期0.5)から `d_b = min(公称長, 音符長 × 取り分上限)`、
  語頭が両唇音でなければ `d_b = 0`(音符区間の全体を母音で均等分割する)。取り分上限は、短い音符で
  閉鎖が音符の大半を占めて母音が潰れるのを防ぐ。公称長・取り分上限の数値は視覚で詰める。
