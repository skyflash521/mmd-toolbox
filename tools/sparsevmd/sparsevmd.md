...
- カメラの perspective(透視投影フラグ)が切り替わるフレームは、閾値によらず
  ...
  またいで補間曲線を作らない。
- 検出位置は `--dry-run` と verboseログに出力する。
...
末尾が0)。verbose ログは `range`・`iterations`・`added_total` に
...
11. 視野角量子化: `--camera-fov-tol` 0.5未満が引数エラーとなり、整数丸め誤差が
    追加分割を誘発しないこと
