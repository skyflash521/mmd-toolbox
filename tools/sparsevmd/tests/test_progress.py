...
ProgressReporter は重い処理(キーフレーム圧縮)の進行を stderr へ1本のライブ行で表示する(TTY時のみ)。
...
update に note スロットを持たせ、描画行へ note を併記する(sparsevmd では現在処理中の対象——camera・
ボーン名等——の注記に使う)。stderr 書き込みが失敗(エンコード不能等)しても例外を外へ漏らさず
enabled=False に落ち、以後 no-op。書き込み失敗で表示が無効化された後も、close はハートビートスレッドを
必ず停止・join する。
...
from sparsevmd import progress
...
def test_stages_share_one_line_without_newline():
    ...
    r._draw()
    r.stage("キーフレーム圧縮")  # bone段(同じラベルでも stage() 呼び出しで新しい段として経過をリセット)
