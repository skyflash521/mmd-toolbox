...
def test_pitch_is_rounded_to_the_nearest_semitone():
    ...
    assert notes.split(track, _one_vowel(track))[0].midi == 69
...
def test_all_unvoiced_produces_no_note():
    ...
    assert notes.split(track, _one_vowel(track)) == []
...
def test_pitch_change_splits_the_note():
    ...
    assert [note.midi for note in notes.split(track, _one_vowel(track))] == [69, 71]
...
def test_gap_alone_does_not_split_the_note():
    ...
    assert len(notes.split(track, segments)) == 1
...
def test_same_input_gives_the_same_notes():
    ...
    first = notes.split(track, segments)
    second = notes.split(track, segments)
