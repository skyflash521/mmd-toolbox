...
| `-v, --verbose` | off | 通常実行でも `--dry-run` と同じ診断を標準出力へ表示する(出力 vpr は書く)。機械モードでは標準出力をイベント専用に保つためこの人間向け表示は出さない([§9](#9-機械モード機械可読インターフェース)。イベント内容も変えない) |
...
- **診断用の中間生成物**: `--keep-intermediate` 未指定時、`song2vpr` は診断用の中間生成物を追加でディスクへ
  ...
  保存先を出力に隣接する `<出力ファイル名>.intermediate/` に決めて音声前段の書き出しを有効にする
