...
日本語の歌の音声ファイル(BGM込み可)から、ボーカル分離・音素認識・音量解析を経て、
リップモーション VMD を自動生成するツール。
...
これらの依存(`soundfile` を含む)が入らない。その状態で音声ファイルを渡して実行すると、
...
使えるオプションの一覧と短い説明は `song2vmd --help` で確認できる。
...
| `--model-name NAME` | `song2vmd <バージョン>` | VMD に格納するモデル名(最大20バイト・Shift-JIS)。 |
...
置き換えたい場合のセットアップ。**既定のままならこのセットアップは一切不要。**
...
- **SOFA は非ASCII文字(日本語など)を含むパスを扱えない**。SOFA 本体の置き場所・チェックポイントの
  置き場所は ASCII のみのパス(例: `C:\tools\sofa`)にする。加えて `song2vmd` が
...
| チェックポイント(`.ckpt` ファイル) | 利用者が配布元から入手する([チェックポイントのライセンス](#チェックポイントのライセンス))。大きさは配布元により約53MB〜約1.2GB |
...
7. **`song2vmd` から使う**。SOFA 専用 venv を抜け、`song2vmd` 本体の venv(トップ [README.md の 5. ツールを実行する](../../README.md#5-ツールを実行する)
   で有効にしたもの)に切り替える:
