...
日本語の歌の音声ファイルから、標準表情モーフ(あ・い・う・え・お・ん)によるリップモーションVMDを
自動生成する独立CLIツールの要求仕様。
...
- 解剖学的な口形合成や、子音ごとの精密な口形再現は行わない。あくまで標準口モーフ(母音と撥音「ん」)によるリップモーション。
...
入力 音声ファイル(歌, BGM込み可)
  ...
  └─[S4]    モーフ生成       lipsync(共有)+ vmd → あ・い・う・え・お・ん のリップモーションVMD
```
...
(TTY 判定に依存させない。12章)。`recognize` 段では、内容認識モデル・音素モデルの初回取得が実際に
ネットワークダウンロードを要した区間だけ、その進捗(取得中のモデル・取得済み割合)を段の補足へ都度
反映し、取得が終わり次第補足を消す(キャッシュ済みの取得では補足を出さない。12.1)。
...
- **開口減衰種別の付与**: 各母音イベントには、顎の狭め具合による開口量の減衰(`lipsync` の ApertureClass)を、
  ...
  一方の判定が他方の結果に影響しない)。直前の区切り(母音・gap・撥音「ん」・両唇閉鎖のいずれか、または
...
- **決定論**: 入口処理(あいうえお写像・両唇閉鎖判定・撥音「ん」判定・開口減衰種別の判定・gap解決・母音区間の
...
- **モーラ代表RMSの声量レンジ再正規化(song2vmd 入口)**: 6.4のモーラ代表RMSは曲全体のフレームRMS基準
  ...
  無音/発声の分離には有効だが、発声(母音・撥音「ん」)区間だけを取り出すと正規化後の上端に偏りやすい。
  開き量の決定にだけ使う値として、曲全体の全モーラ(母音・撥音「ん」)の代表RMS集合に対し、
...
- **長時間モーラのサブウィンドウ分割**: 歌唱で長く伸ばすモーラ(ロングトーン・メリスマ)は、その区間内で
  ...
  強弱節点として扱う)。
  - **対象と閾値**: モーラ(母音・撥音「ん」)の区間長(終了時刻マイナス開始時刻、秒)が閾値(初期値1.0秒)
...
- **progress**: `{type:"progress", stage, done, total, note, elapsed}`。`stage` は処理段階の安定 id:
  ...
  各段は開始時に最低1本の progress を出す。`recognize` 段は、内容認識モデル・音素モデルの初回取得が
  実際にダウンロードを要した区間だけ、`done`/`total`(長尺分割時はそのチャンクの値のまま)を保った
  追加の progress を都度出し、`note` へ取得中のモデルと取得済み割合を反映する。取得が終わり次第
  `note` を空文字列に戻す1本を出す(キャッシュ済みの取得ではこれらの追加 progress 自体を出さない)。
