...
| `--separator NAME` | `demucs` | S1ボーカル分離バックエンドの選択(7章)。値は vocal_analysis の登録アダプタの安定 id(vocal_analysis.md §8。現行は `demucs` のみ) |
| `--recognizer NAME` | `wav2vec2-espeak` | S2音素/母音認識バックエンドの選択。値は vocal_analysis の登録アダプタの安定 id(vocal_analysis.md §8。現行は `wav2vec2-espeak` のみ) |
