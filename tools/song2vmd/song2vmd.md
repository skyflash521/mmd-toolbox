...
- **観測できる音響イベントで規則を書く**: 文字列が無いため、字面上の特殊モーラ(促音・語末など)を音響
  ...
  - **閉鎖・無音区間**: 子音セグメントがま行・ば行・ぱ行の頭子音(採用構成の音素記号で `m`/`mʲ`・`b`/`bʲ`・
    ...
    丸める子音)ため閉鎖にしない。gap 区間は RMS で判定し、RMSが無音しきい値以下なら無音(閉口)、
    発声が続いていれば直前母音を継続する(低ダイナミクス曲ではこの音量に基づく無音化を抑制。6.4)。
    直前母音が無い先頭の gap と末尾の gap は、RMSに依らず閉口とする(口は閉口で始まり閉口で終わる)。
