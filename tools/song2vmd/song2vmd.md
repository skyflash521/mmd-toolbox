...
- **開き量の決定(song2vmd 入口。RMS→開き量の写像)**: 各モーラ区間の再正規化後RMS(上記)を
  ...
  目安。8章)へクランプして、そのモーラの開き量(保持値スケール、0〜1相当)を決める。声が強いモーラ
...
- **長時間モーラのサブウィンドウ分割**: 歌唱で長く伸ばすモーラ(ロングトーン・メリスマ)は、その区間内で
  ...
  - **サブウィンドウ代表RMS**: 各サブウィンドウ区間の中央60%窓平均RMS(6.4と同じ中央60%窓)とする。6.4の
    ...
    にはこの文脈が無いため)。
  - **正規化と写像**: 各サブウィンドウの代表RMSは、曲全体の全モーラ(母音的口形の各モーラにつき、分割の
    有無によらず1つずつの代表RMSからなる母集団。サブウィンドウの代表RMSはこの母集団に含めない)から求めた
    パーセンタイル境界(p10/p90)を使って個別に線形正規化し、RMS→開き量の写像(`--intensity-curve`の累乗則・
    開き量レンジ・`--open-max`へのクランプ)をサブウィンドウごとに適用して、サブウィンドウごとの開き量を
    決める。
