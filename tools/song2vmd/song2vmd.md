...
- **観測できる音響イベントで規則を書く**: 文字列が無いため、字面上の特殊モーラ(促音・語末など)を音響
  だけから一般には確定できない。撥音「ん」は後続母音を持たない鼻音セグメントとして音響的に観測できるため、
  その音響イベントから確定する(下記)。よって口形イベントの確定は次の観測可能なイベントに基づく:
  ...
  - **撥音「ん」区間**: 後続母音を持たない鼻音子音セグメント(IPA の m/n/ŋ/ɲ/ɴ 等で、直後が母音セグメントで
    ない=次が gap・別子音・末尾のもの)を撥音「ん」(`MouthShape.N`)にする。音節末の鼻音は口を閉じ気味に保つ
    有声の鼻音で、閉口や母音には倒さず「ん」モーフを正に立てる(lipsync.md §2.1)。母音が直後に続く鼻音は頭子音
    (な行・ま行など)なので撥音にしない(ま行頭子音は下の両唇閉鎖、その他の頭子音は独立イベントを作らず協調調音へ
    委ねる)。撥音の開き量はその区間の代表RMSから決める(閉じ気味なので通常は控えめ。6.4)。`--no-n-morph`(5.2)
    指定時は撥音「ん」を無音(閉口)に倒す。
  - **閉鎖・無音区間**: 子音セグメントが**後続母音を持つ**両唇閉鎖音(ま行・ば行・ぱ行の頭子音。IPA では母音が
    直後に続く m・b・p と、ば行の摩擦異音 β)なら一瞬の閉口にする(後続母音を持たない音節末の鼻音は上の撥音「ん」)。
    ɸ(ふ)は両唇音だが閉口しない(唇を丸める子音)ため閉鎖にしない。gap 区間は RMS で
    判定し、RMSが無音しきい値以下なら無音(閉口)、発声が続いていれば直前母音を継続する(低ダイナミクス曲では
    この音量に基づく無音化を抑制。6.4)。直前母音が無い先頭の gap と末尾の gap は、RMSに依らず閉口とする
    (口は閉口で始まり閉口で終わる)。促音か無音かを字面で区別せず、観測した無音/閉鎖の長さで扱う。
  - **弱い/無声の母音区間**: 認識信頼度とRMSが低い母音区間は、開き量を弱めて `lipsync` に渡す(短すぎる区間は
    lipsync の最小保持で前後へ吸収される。6.5)。判定は、認識器が信頼度を出す場合は 信頼度 < 0.5 かつ
    正規化RMS < 0.3(初期値)、信頼度を出さないバックエンドでは 正規化RMS < 0.2(初期値)とし、該当区間の
    開き量を 0.5 倍(初期値)に弱める。しきい値・倍率は実データで調整する。
...
- **先頭子音種別の付与**: 各母音イベントには、唇に影響する子音の口形変調のため、先頭子音種別
  ...
  ɸ・w(ふ・わ)は ROUNDED、ɕ・tɕ・ʑ・dʑ・ɲ および硬口蓋化(ʲ 付き)の子音(し・ち・じ・拗音)は SPREAD、
  その他の子音は NEUTRAL、先行する隣接子音が無ければ NONE。判定表に無い子音は NEUTRAL に倒す
  (唇を動かさない扱い)。
- あいうえお写像の適用・閉鎖判定・gap解決・観測イベント規則・口形イベント確定は日本語固有の入口処理として
  `song2vmd` 側に置く(vocal_analysis は写像規則と音素セグメント列・RMS を提供するまで)。
- **適用順(決定論)**: 入口処理は (1) IPA→あいうえお写像・両唇閉鎖判定・撥音「ん」判定(鼻音子音を後続母音の
  有無で頭子音/撥音へ振り分け)、(2) gap解決と母音区間の無音補正
...
- 分割は歌唱の途中で切らないよう、無音区間で区切る。分割境界は、目標境界の前後 ±5秒(初期値)の探索窓で
  最も静かな点を選ぶ。探索窓内に無音が無い場合(ロングトーン・ライブ音源等)は最大チャンク長
...
| アタック | 2 | 3 | 1 | 2 | 1 |
| リリース | 2 | 3 | 1 | 2 | 1 |
| 協調調音の重なり上限(`--coarticulation`) | 2 | 3 | 2 | 2 | 1 |
| 先行(`--anticipation`) | 1 | 1 | 2 | 1 | 1 |
| 最小保持(`--min-hold`) | 3 | 4 | 3 | 3 | 2 |
| 合成誇張係数 | 1.0 | 0.8 | 1.3 | 0.7 | 1.0 |
| 開き量上限(`--open-max`) | 0.90 | 0.70 | 0.97 | 0.50 | 0.85 |

プリセットに依らない共通の初期値: 強弱→開き量の累乗指数(`--intensity-curve`)は 0.6、母音別の開き量倍率
(`--vowel-gain`)は 1:1:1:1:1、無音ヒステリシス(`--silence-threshold`)は 開始(下降側)0.06・終了(上昇側)
...
口形に関与するのは母音別の開き量倍率 `--vowel-gain` と開き量レンジ・上限(8.1・8.3)であって、
母音→モーフ重みの合成規則そのものではない。`lipsync` の `vowel_scale` は母音的口形別(a, i, u, e, o, ん)の
6要素倍率(lipsync.md §4.8)なので、`song2vmd` は `--vowel-gain` の5母音値に撥音「ん」の倍率 1.0 を補って
`(a, i, u, e, o, 1.0)` の6要素として渡す。撥音の開き量は `--vowel-gain` で個別調整せず、その区間のRMSと
「ん」プロファイルで決まる(6.3・6.4)。
...
- 母音別の開き量倍率(`--vowel-gain`)。
- 無音判定のヒステリシス開始/終了しきい値(`--silence-threshold`)、両唇音の閉口の深さ・長さ。

長い伸ばし音の揺らぎ(伸び表現)は `lipsync` の生成既定([lipsync.md](../../libs/lipsync/lipsync.md) §4.6)を
そのまま使い、プリセット・CLI からは上書きしない。
...
- **出力の原子性**: VMD 出力は一時ファイルへ書き切ってから最終パスへ置換する(`vmd.io`。5.3)。途中終了で
  中途半端な出力ファイルを残さない。中間生成物は一時作業ディレクトリにあり(5.3)、最終出力先を汚さない。
