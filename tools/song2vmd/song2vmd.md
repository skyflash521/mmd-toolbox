...
| モーラ境界の谷(半幅/最低間隔) | 1.0/4.0 | 1.0/4.0 | 1.0/4.0 | 1.0/4.0 | 1.0/4.0 |

`pop` の母音別倍率・先行・協調調音・伸び表現・レガート谷は、同じ `lipsync` 生成コアを使う口パク
生成系のMMD視覚チューニングで確定した標準値。他スタイルは未チューニングの出発点値で、必要に
なったら個別に視覚で詰める。モーラ境界の谷は `pop` を含む全プリセットが `lipsync` の初期目安
(lipsync.md §4.8)のまま未チューニングで、実データの目視で詰める。プリセットは母音別倍率を含む上記の生成パラメータ一式を明示的に持ち、
...
長い伸ばし音の揺らぎ(伸び表現)・レガート谷・三角形短区間の下限・モーラ境界の谷(半幅・最低間隔)は
プリセットが具体値を持ち(8.1)、CLI からは上書きしない。
