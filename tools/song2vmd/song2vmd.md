...
- **閉じ側の境界調整**: 認識構成の境界は母音の開始側が正確で、終端(閉じ側=後続の子音・無音への切り替わり)は
  近似となる(vocal_analysis.md §9)。閉じ側のタイミングは入口の調整点とする: 子音開始の先行量(閉じの先行。
...
- 母音認識は歌唱で誤ることがある。認識方式・認識構成の選定と品質測定は vocal_analysis が正本
  (vocal_analysis.md §5・§9)。認識の時刻は開始側が正確で母音の終端(閉じ側)は近似のため、境界は入口の
  RMSオンセット補正・閉じ側調整で補正する(6.3・6.4)。将来、任意の歌詞テキスト指定による精度向上の
  余地を残す。
