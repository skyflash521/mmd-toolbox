...
ダウンロードが発生しない場合を含む)にロード中である旨を補足へ反映し、ロード完了までの区間を
無補足にしない。加えて、モデル取得の有無に関わらず、`separate` 段は分離処理(Demucs推論)の進捗、
`recognize` 段は書き起こし・強制アライメントが処理中の区間の時刻範囲を段の補足へ都度反映する
(12.1)。
