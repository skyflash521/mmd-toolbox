...
| `--vowel-gain a:i:u:e:o` | `1:1:1:1:1` | 母音別の開き量倍率(口形バランス調整。プリセット非依存の共通既定。8.1) |
...
- **観測できる音響イベントで規則を書く**: 文字列が無いため、字面上の特殊モーラ(撥音・促音・語末など)を音響
  ...
  - **閉鎖・無音区間**: 子音セグメントが両唇閉鎖音(ま行・ば行・ぱ行。IPA では m・b・p と、ば行の摩擦異音
    β)なら一瞬の閉口にする。ɸ(ふ)は両唇音だが閉口しない(唇を丸める子音)ため閉鎖にしない。gap 区間は RMS で
...
- **適用順(決定論)**: 入口処理は (1) IPA→あいうえお写像と両唇閉鎖判定、(2) gap解決と母音区間の無音補正
...
- **開き量の決定(song2vmd 入口。RMS→開き量の写像)**: 各モーラ区間のRMS(6.4)を `--intensity-curve` の
  ...
  (lipsync.md §2.2・§6)。
- **lipsync へ渡す入力**: 6.3 で確定した口形イベント列(母音 a/i/u/e/o・両唇閉鎖・無音/休符)、各母音
