...
誇張係数・開き量上限)を確定し、CLIで明示指定された値(--open-max・--coarticulation・
--anticipation・--min-hold)があればプリセット値より優先する presets.resolve を検証する。
"""
...
# 先行・最小保持・誇張係数・開き量上限)。
_EXPECTED = {
    "pop": (0.30, 0.75, 2, 2, 2, 1, 3, 1.0, 0.90),
...
def test_different_styles_give_different_values():
    # pop/whisperはmin_hold_framesが同じ3(song2vmd.md 8.1)なのでこの2項目では比較しない。
