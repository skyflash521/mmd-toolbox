...
def _weights_at(events, params=None):
    ...
    return {k.name: k.weight for k in keys}
...
def test_single_morph_vowel_a():
    ...
    assert _weights_at(_vowel(MouthShape.A, 0.5)) == pytest.approx({"あ": 0.5})


def test_composite_vowel_i_multiple_morphs():
    # い: {あ:0.1, い:1.0}。hold=0.5 → {あ:0.05, い:0.5}、総量0.55<cap。
    w = _weights_at(_vowel(MouthShape.I, 0.5))
    assert set(w) == {"あ", "い"}
...
def test_hold_clamped_to_open_cap():
    ...
    assert _weights_at(_vowel(MouthShape.A, 1.0)) == pytest.approx({"あ": 0.8})
...
def test_vowel_scale_applied_then_clamped():
    ...
    assert _weights_at(_vowel(MouthShape.A, 0.5), p) == pytest.approx({"あ": 0.8})
...
def test_total_clamp_proportional_scaling():
    ...
    w = _weights_at(_vowel(MouthShape.E, 1.0))
    factor = 0.8 / 1.12
...
def test_open_cap_param_used_for_hold_clamp():
    ...
    assert _weights_at(_vowel(MouthShape.A, 1.0), p) == pytest.approx({"あ": 0.6})
...
def test_open_cap_param_used_for_total_clamp():
    ...
    p = GenerationParams(open_cap=0.6)
    w = _weights_at(_vowel(MouthShape.E, 1.0), p)
    factor = 0.6 / 0.84
...
def test_exaggeration_scales_aux_only():
    ...
    w = _weights_at(_vowel(MouthShape.I, 0.5), p)
...
def test_vowel_scale_indexed_per_vowel(shape, index, expected):
    ...
    assert _weights_at(_vowel(shape, 0.3), p) == pytest.approx(expected)
...
def test_profile_known_weights(shape, expected):
    ...
    assert _weights_at(_vowel(shape, 0.4)) == pytest.approx(expected)
