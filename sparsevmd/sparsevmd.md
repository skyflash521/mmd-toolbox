...
3. `--max-segment-frames` は**出力キー間隔の sliding 上限**として扱う。未指定なら上限なし(無制限)で、
   ...
   指定したときは、誤差と無関係な等分点を無条件の出力キーにはしない(機械的な等分 grid キーを生まない
   ため)。間隔の上限維持は区間処理(step 6)で、確定キーから次キーまでが上限を超える非定数区間に限って行う。
...
6. すべての誤差が許容値以下で、かつ区間長が `--max-segment-frames` 以下(または全チャンネル定数、
   または `--max-segment-frames` が無制限)なら区間を採用する。誤差は許容内でも区間長が上限を超える
   非定数区間は(上限が有限のときに限り)、上限を守るため `maxspan-cap`
