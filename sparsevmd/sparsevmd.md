...
`bezier` モードでは、出力後検証(§7.3)のループ内で、出力キーを構築したあとに**位置チャンネルの
補間制御点へ端点速度平滑化ポストパスを適用する**。疎化は区間ごとに独立にフィットするため内部キーで
補間速度が不連続になりがちで、再生時にキー境界が速度のカクつき(コーナー)として見える。これを
軽減するため、各内部キー `k` で、到達側区間 `[k-1,k]` の終端と出発側区間 `[k,k+1]` の始端のベジェ
端点速度を、ソースの中心差分速度(キー `k` 前後フレームの値差/2)へ近づける。両隣を同じ速度へ揃えると
キーで速度が連続(C1)になる。

制御点は §5.4 と同じく `[0,127]` へクランプする。クランプにより、必要な速度が大きい区間や正味変位と
逆符号の(局所極値付近の)区間では端点速度を合わせきれず実現可能範囲への射影にとどまるため、これは
厳密な C1 保証ではなく **C1 近似**である。クランプは曲線の単調性(X・Y とも単調)を保つので出力は常に
MMD 補間曲線として適合する。

許容契約は破らない:
- 非strict: C1 適用後に §7.3 の検証を行い、許容超過があれば `verification-add` で密化して収束させる。
- `--strict`: C1 適用後に許容超過する範囲では C1 を適用せず元フィットを残す(C1 が `--strict` の許容
  失敗を新たに誘発しない)。
