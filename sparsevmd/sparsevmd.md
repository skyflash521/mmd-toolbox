...
`--range` 指定時、対象トラックの範囲外キーは元の値と補間曲線を保持する。
範囲端が対象トラックの既存キー上にない場合は、内部作業ビュー上で端フレームを
サンプリングして境界キーを追加する。境界追加により範囲外の隣接区間も補間曲線を
書き換える必要があるため、`--dry-run` と verbose ログで必ず報告する。
...
- `--range` 外の対象キーのうち、削減区間に隣接しないキーは元のフィールド値と
  補間曲線を保持する。削減区間に隣接する区間では、範囲端の境界キー追加により
  到達側補間曲線が書き換わる場合がある。
- 対象外セクションはバイト単位で保持する。
...
- 範囲外の対象キーを元の値と補間曲線で保持する場合、範囲端の到達側補間曲線が
  範囲外区間に影響することがある。この場合は境界キー追加と補間曲線書き換えを
  レポートに明示する。
...
8. 範囲指定: `--range` 外の対象キー保持、境界キー追加、レポート出力が仕様通りであること
