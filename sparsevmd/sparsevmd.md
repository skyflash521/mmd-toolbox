...
| 引数 | 型 | デフォルト | 説明 |
|---|---|---|---|
| `--max-segment-frames` | int | 180 | 1本の補間区間で試す最大フレーム長。超える場合は事前分割 |
...
3. 必須キー間の区間を `--max-segment-frames` 以下に事前分割する
4. 各区間について、全チャンネルグループがVMD補間曲線で表現可能か試す
5. 量子化済み補間曲線で出力候補を作り、整数フレームで再サンプリングして誤差を測る
6. すべての誤差が許容値以下なら区間を採用する
7. 許容値を超えた場合、最大誤差フレームで区間を分割し、再帰的に処理する
...
- `bezier` モードは高品質だが `linear` より計算量が大きい。内部点を持つ候補区間では最小二乗で
  制御点を探索し(`least_squares` を最大4初期値で、許容内に収まり次第打ち切って試す。内部点が
  無い区間は線形制御点で探索しない)、出力後検証(§7.3)で許容超過があれば区間を密化して再フィット
  するため、長尺・高密度入力ではフィット回数が増えて処理時間が伸びる。
...
- フィットコストは「小区間フィットの数 × `least_squares` 評価回数」が支配的で、内部点ごとの
  曲線評価ループ長ではない。実入力は小区間(特に `max-segment-frames` が小さいと内部点数個)が
  支配的なため、曲線評価を numpy で配列化しても固定オーバーヘッドが上回り高速化しない。
  代表的な高密度カメラ入力の計測では、曲線評価をベクトル化すると `balanced` 中レンジと
  `aggressive`/max5 中レンジがともに約14%遅化し、出力キー数・誤差はビット一致した(品質不変で
  速度のみ悪化)。速度を詰める場合は評価回数側(初期値数・反復上限)を見直す。
