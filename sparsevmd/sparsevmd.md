...
3. 必須キー間の区間を `--max-segment-frames` 以下に事前分割する。
   ただし**全チャンネルが定数(無変化)の区間は事前分割の対象外**とする。`--max-segment-frames` は
   曲がった区間を手編集しやすく短く保つための上限で、定数区間には編集すべき曲がりが無く、
   等分はキー過多とフィット評価増を招くだけのため。定数判定は各チャンネルが自分の値型・単位で、
   区間内のサンプルが量子化下限相当のしきい値以下しか動かないかを見る。定数判定の手段を持たない
   チャンネルが1つでもあれば従来どおり等分する
