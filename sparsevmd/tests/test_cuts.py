"""不連続検出・必須境界のテスト(sparsevmd.md §2.6, §6.1, §6.2)。
...
from sparsevmd import cuts
from sparsevmd.cuts import (
    assemble_boundaries,
    detect_cuts_bone,
    detect_cuts_camera,
    parse_cut_threshold_bone,
    parse_cut_threshold_camera,
...
def test_camera_rotation_wraparound_not_detected():
    # 179度→-179度 は見かけ上 358度差だが実角度差は2度。最小角差で誤検出しない(§6.1)。
...
def test_camera_threshold_exact_value_no_cut():
    # 「超えた場合」=厳密に超過(§6.1)。ちょうど閾値ならカットしない。
