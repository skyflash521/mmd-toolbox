...
def test_upper_seam_curve_preserved():
    ...
    # 継ぎ目は元曲線の部分弧。単一区間を整数量子化制御点1本で再フィットするため、正規化誤差は
    # 量子化下限(約0.005)まで下がるが、区間の値スパン(約93)が乗るので position 単位では
    # 約0.45 残る。それでも書き換えなしの約25から大幅に改善する。継ぎ目を tol ぴったりに収める
    # 密化(範囲外への境界キー追加)は本実装のスコープ外。
    assert _max_seam_dev(out, src, 20, 30) < 1.0
