...
#
# 未実装の skip_fastpath 経路を呼ぶ直前に pytest.xfail() を置く(実装後に外す)。
# こうすると既存挙動の前提確認(既定でファストパス発火など)は通常実行され、目的外の回帰が
# xfail に隠れない。前提を持たない自明ケース(空区間)はデコレータ xfail でよい。
_PENDING = "impl pending: fit skip_fastpath"
