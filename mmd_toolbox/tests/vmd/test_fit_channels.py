...
from mmd_toolbox.vmd.fit import EuclideanVectorChannel, FovChannel, _round_half_up
...
def test_vector_prefers_velocity_reversal_over_max_error():
    ...
    # 速度反転(切り返し)は frame1/frame2。§5.5 に従い分割候補は反転中の誤差最大 frame2。
...
def test_round_half_up_is_not_bankers():
    # 四捨五入(0.5切り上げ)。Python 標準 round の偶数丸めとは異なる(§3.2)。
...
def test_fov_prefers_velocity_reversal_over_max_error():
    ...
    # frame1/frame2。§5.5 で分割候補は反転中の誤差最大 frame2。採否用最大誤差は11。
