"""疎化フィットの性能ガード(mmd_toolbox.md §6.4)。
...
# mmd_toolbox/tests/vmd/<this> から見たリポジトリルート(サブプロセスの cwd。sys.path[0]=cwd
# となり、site-packages の古いコピーでなくソースの mmd_toolbox を import する)。
...
def test_bezier_reduce_does_not_hang():
    try:
        ...
        pytest.fail(
            ...
            " 曲線評価のベクトル化など棄却済み最適化を踏んでいないか mmd_toolbox.md §6 を確認。"
        )
