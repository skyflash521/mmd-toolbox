...
「試して棄却した最適化」は mmd_toolbox.md §6 を正とする。曲線評価(`_bezier_y_at`/`residual`)の
per-sample numpy ベクトル化は棄却済み(小区間で固定オーバーヘッドが上回り遅化。§6.2)。
高速化するなら評価回数側(`least_squares` の呼び出し数・反復)を削る(§6.3)。
"""
