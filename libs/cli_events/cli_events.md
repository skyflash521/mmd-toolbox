...
- **1 行 1 オブジェクト**: 各イベントを単一行の JSON として書き、行末で改行する。行区切りは **LF(`\n`)固定**
  ...
  避け `\r` を混入させない(規約 §10)。JSON 内に生の改行を入れず、呼び出し側が逐次パースできるようにする。
...
引数エラーなので、**終了コードは規約 §5 の基底共通の `2` で固定**する(基底 `0`〜`3` の意味は全ツール共通で
各ツール裁量にしない。規約 §6)。一方、安定 `code` 値・該当 `field`・`message` は各ツールが定めるので、
