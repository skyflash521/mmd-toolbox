"""S3 強弱エンベロープ(vocal_analysis.md §6・§6.1)。

ボーカルWAVのRMSエンベロープを numpy/scipy で算出する。外部ツールに依存しない。フレーム化とパーセンタイル
相対正規化・dynamic_range_db 算出はいずれも入力ゲインに不変(3章のピーク正規化と同じく、正の一様ゲインに
対して結果が変わらない)。
"""
...
def _frame_rms(samples: np.ndarray, sample_rate: int) -> tuple[np.ndarray, np.ndarray]:
    """フレーム化した生RMS(パーセンタイル正規化前)と各フレーム中心時刻を返す(§6.1)。
...
def _normalize_rms(raw_rms: np.ndarray) -> np.ndarray:
    """曲全体のパーセンタイル(p10/p90)で相対正規化する(§6.1)。"""
    if raw_rms.size == 0:
        return raw_rms
...
def _dynamic_range_db(raw_rms: np.ndarray) -> float:
    """曲全体の95/5パーセンタイル RMS の dB 差を算出する(§6.1)。"""
    if raw_rms.size == 0:
        return 0.0
