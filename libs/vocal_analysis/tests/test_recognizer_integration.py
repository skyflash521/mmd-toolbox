...
# vocal_analysis.recognizer に統合部分(recognize等)が未実装の間は import が失敗するため
# xfail 印で緑を保つ。strict=True: 未実装印を外し忘れたまま通ると XPASS が失敗になり検出できる。
pytestmark = pytest.mark.xfail(reason="impl pending: vocal_analysis.recognizer integration", strict=True)
...
def test_load_model_and_processor_passes_pinned_config(monkeypatch):
    # transformers が実際に導入されている環境でのみ、_load_model_and_processor の実体を検証する
    # (最小環境では skip)。from_pretrained 自体をモンキーパッチするためネットワーク・実モデルの
    # ダウンロードは発生しない。
    ...
    def fake_model_from_pretrained(model_id, revision=None, **kwargs):
