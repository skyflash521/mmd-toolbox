...
- **内容認識(文字起こし)は書き起こされた区間では歌唱の内容をおおむね再現するが、長尺の単一呼び出しでは
  ...
  制約源としては有効であることを実測で確認済み([vocal_analysis.md §5.2](vocal_analysis.md#52-wav2vec2-ctc経路-複合構成内容認識g2p強制アライメントの区間化) 手順3・手順7)。また、
  ...
  区間ごとに独立して Whisper を呼ぶ([vocal_analysis.md §5.2](vocal_analysis.md#52-wav2vec2-ctc経路-複合構成内容認識g2p強制アライメントの区間化))。
- **強制アライメントは歌唱でも時刻をある程度正確に復元できるが、素朴な一括適用には落とし穴がある**。
  ...
  ([vocal_analysis.md §5.2](vocal_analysis.md#52-wav2vec2-ctc経路-複合構成内容認識g2p強制アライメントの区間化))。「正しい歌詞が前提」という強制アライメントの弱点は、
...
> (歌唱では開始側にもフレーズ内のずれがある。[vocal_analysis.md §5.2](vocal_analysis.md#52-wav2vec2-ctc経路-複合構成内容認識g2p強制アライメントの区間化) の既知の限界)。母音区間の境界の精緻化
...
1. **既定値は歌唱データで検証済み・高速**。`openai/whisper-medium`+かな限定プロンプトは既存のS-1
   ...
   残りうる([vocal_analysis.md §5.2](vocal_analysis.md#52-wav2vec2-ctc経路-複合構成内容認識g2p強制アライメントの区間化))。
...
4. **G2P(`pyopenjtalk-plus`)はどちらの内容認識モデルの出力に対しても同一の呼び出しで足りる**。入力が
   ...
   読みや長音の解釈などかなでも文脈依存のケースはわずかに残る([vocal_analysis.md §5.2](vocal_analysis.md#52-wav2vec2-ctc経路-複合構成内容認識g2p強制アライメントの区間化))。
...
強制アライメント段(手順5以降相当)だけを置き換える選択肢として、[`vocal_analysis.md` §5.3](vocal_analysis.md#53-sofa経路-単語単位アライメント)・[§8.3](vocal_analysis.md#83-採用ツールと代替候補)の
...
- **フルパイプライン比較での乖離**: 内容認識→G2P→強制アライメント→VMD生成のフルパイプラインで
  ...
  [`vocal_analysis.md` §8.3](vocal_analysis.md#83-採用ツールと代替候補)で両アダプタを選択可能なまま維持し既定を変更しない理由の根拠である。
- **チェックポイントのライセンス(既定値を持たない理由)**: 入手可能な日本語SOFAチェックポイント
  ...
  意図せずライセンス違反を犯しうるため、[`vocal_analysis.md` §8.3](vocal_analysis.md#83-採用ツールと代替候補)のとおりこの2件のいずれも既定値には
  しない(利用者保護のための方針判断)。
...
### 2.4 英語カタカナ化フォールバックの候補([vocal_analysis.md §5.2](vocal_analysis.md#52-wav2vec2-ctc経路-複合構成内容認識g2p強制アライメントの区間化)手順4)
...
- 英語カタカナ化フォールバック([vocal_analysis.md §5.2](vocal_analysis.md#52-wav2vec2-ctc経路-複合構成内容認識g2p強制アライメントの区間化)手順4)関連:
