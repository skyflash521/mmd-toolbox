...
[vocal_analysis.md §8](vocal_analysis.md#8-外部ツール連携機構)・[§2](vocal_analysis.md#2-共有出力正規化中間形式) を正とする。採用は実データでの品質・導入性の評価で見直しうる
(見直す場合は正本を先に更新する)。

## 0. 前提と評価軸

差し替え可能な委譲ステージ([vocal_analysis.md §2](vocal_analysis.md#2-共有出力正規化中間形式)): S1 ボーカル抽出 / S2 音素・母音認識。
...
- **書き起こしテキストを製品の入出力にしない**。後段に必要なのは「母音・音素の時刻」であり単語ではない。
  書き起こしテキストは共有出力に含めず、歌詞テキストの入力も要求しない([vocal_analysis.md §5](vocal_analysis.md#5-音素母音認識s2))。
  ただし認識構成が**内部で**内容認識(文字起こし)を音素列の情報源として使うことは、この方針に反しない([§2](#2-s2-音素母音認識))。
...
`mode=never` で扱う([vocal_analysis.md §8.1](vocal_analysis.md#81-アダプタinterface))。

ボーカル抽出ツールの切り替えは [vocal_analysis.md §8.1](vocal_analysis.md#81-アダプタinterface) の **Separator 抽象**の背後で行う。
...
- **内容認識(文字起こし)は書き起こされた区間では歌唱の内容をおおむね再現するが、長尺の単一呼び出しでは
  ...
  文字起こしできる(軽微な単語誤りは残る。精度の具体的な数値評価はS-1測定=[vocal_analysis.md §9](vocal_analysis.md#9-認識品質の測定s-1)に委ねる)。単語レベルの
  タイムスタンプは、直接の時刻源として単体で使うには粗いが、±0.75秒程度の余白を持つ強制アライメントの
  制約源としては有効であることを実測で確認済み([vocal_analysis.md §5.2.3](vocal_analysis.md#523-内容認識)・[vocal_analysis.md の単語窓制約の余白の決定](vocal_analysis.md#単語窓制約の余白の決定))。また、
  ...
  区間ごとに独立して Whisper を呼ぶ([vocal_analysis.md §5.2.1](vocal_analysis.md#521-区間分割無音検出)・[vocal_analysis.md §5.2.3](vocal_analysis.md#523-内容認識))。
- **強制アライメントは歌唱でも時刻をある程度正確に復元できるが、素朴な一括適用には落とし穴がある**。
  ...
  での実測)。採用構成はこれに対処するため、単語タイムスタンプに基づく[vocal_analysis.md の単語窓制約](vocal_analysis.md#単語窓制約)を主とし、単語
  タイムスタンプが取得できない場合の[vocal_analysis.md のフォールバック(位置バンド制限)](vocal_analysis.md#フォールバック位置バンド制限)も設ける。「正しい歌詞が前提」という強制アライメントの弱点は、
  歌詞を音声自身から内容認識で得ることで避ける。CTC の出力は音素位置のスパイクであり持続長を持たないため、
  母音の終端(閉じ側)は近似になる(閉じ側の確定は利用先の入口処理の責務。
  [vocal_analysis.md §9](vocal_analysis.md#9-認識品質の測定s-1)・[§10](vocal_analysis.md#10-利用先に残る責務含まないもの))。時刻精度の具体的な数値は測定条件により変わるため
  ここには固定せず、[vocal_analysis.md §9](vocal_analysis.md#9-認識品質の測定s-1) の S-1 測定で確認する。

→ この3点から、S2 の有望構成は**複合構成**とする: 内容認識(Whisper 系)で歌唱内容のテキストを得て、
G2P(pyopenjtalk 系)で音素列へ変換し、CTC 音素モデルのロジット上の強制アライメントで時刻を確定する。
書き起こしは共有出力に出さず、音素列の情報源としてアダプタ内部でだけ使う([§0](#0-前提と評価軸) の方針と整合)。

### 2.2 候補

内容認識には、どのモデルでも共通のかな限定プロンプト([vocal_analysis.md §5.2](vocal_analysis.md#52-wav2vec2-ctc経路-複合構成内容認識g2p強制アライメントの区間化)・[§8.3](vocal_analysis.md#83-採用ツールと代替候補))を渡す(モデルに
よる分岐は無い)。
...
| `kana-whisper` + G2P + CTC強制アライメント(既定値以外は`ContentRecognizerModel`の任意指定で利用可能。定数化しない) | transformers + pyopenjtalk-plus(いずれも in-process) | 強制アライメントで復元(具体的な精度は[vocal_analysis.md §9](vocal_analysis.md#9-認識品質の測定s-1)のS-1測定で確認) | 未評価(歌唱データでの学習・評価実績が無い) | 低(意味の通る別語への丸ごと置換は起きにくい) | kana-whisper MIT / pyopenjtalk-plus MIT(内包の OpenJTalk 系は修正BSD) |
...
> (歌唱では開始側にもフレーズ内のずれがある。[vocal_analysis.md の既知の限界(歌唱でのフレーズ内時刻ずれ)](vocal_analysis.md#既知の限界歌唱でのフレーズ内時刻ずれ))。母音区間の境界の精緻化
> (近傍にRMSオンセットがあるときそれへ寄せる)と閉じ側の確定は利用先(リップモーション生成系の入口)が行う
> ([vocal_analysis.md §5](vocal_analysis.md#5-音素母音認識s2)・[§10](vocal_analysis.md#10-利用先に残る責務含まないもの))。認識器は5母音と子音、未割当(gap)を区別できれば足り(無音/閉口の
> 確定は利用先がRMS併用で行い、両唇閉鎖は音素から判定)、語彙認識より要件は緩い。

**採用: 内容認識+G2P+CTC強制アライメントの複合構成(id・モデル・revision の固定は
[vocal_analysis.md §5.2](vocal_analysis.md#52-wav2vec2-ctc経路-複合構成内容認識g2p強制アライメントの区間化)・[§8.3](vocal_analysis.md#83-採用ツールと代替候補) が正本)。内容認識は既定値 `openai/whisper-medium`(かな限定
プロンプトを渡す)。`kana-whisper` は既定値と並ぶ定数(候補値)としては提供せず、
`ContentRecognizerModel` の任意指定で利用者が直接指定する**。決め手:
...
1. **既定値は歌唱データで検証済み・高速**。`openai/whisper-medium`+かな限定プロンプトは既存のS-1
   ...
   残りうる([vocal_analysis.md の既知の限界(漢字の読み・一部のかなの文脈依存の読み。未軽減)](vocal_analysis.md#既知の限界漢字の読み一部のかなの文脈依存の読み未軽減))。
2. **リップモーション生成系が必要とするのは母音の種類(母音正解率)であり、意味の通る文か否かではない**。内容認識に
   ...
   `kana-whisper` はこの種の語彙置換を起こしにくいが、歌唱データでの学習・評価実績が無いため
   既定値と並ぶ定数(候補値)としては提供せず、必要な利用者が任意指定で選べれば足りる。
...
4. **G2P(`pyopenjtalk-plus`)はどちらの内容認識モデルの出力に対しても同一の呼び出しで足りる**。入力が
   ...
   読みや長音の解釈などかなでも文脈依存のケースはわずかに残る([vocal_analysis.md の既知の限界(漢字の読み・一部のかなの文脈依存の読み。未軽減)](vocal_analysis.md#既知の限界漢字の読み一部のかなの文脈依存の読み未軽減))。
5. **すべて純Pythonでin-process**に呼べ、「利用者にコマンドを叩かせない/ライブラリ呼び出し」方針に合う。
   ...
   [vocal_analysis.md §5.1](vocal_analysis.md#51-母音子音の判定基準とフレーム時間rms不要))。この自動選択は実行時の分岐であり、
...
実装(アダプタ)は [vocal_analysis.md §5.2](vocal_analysis.md#52-wav2vec2-ctc経路-複合構成内容認識g2p強制アライメントの区間化)・[§8.3](vocal_analysis.md#83-採用ツールと代替候補) の確定仕様に従う。最終的な
品質確認は **S-1 認識測定**(構成間の相対比較・破綻検出)と利用先の調整・MMD 上の視聴確認に
従う([vocal_analysis.md §9](vocal_analysis.md#9-認識品質の測定s-1))。**Julius は代替**として残す(歌唱品質か速度が問題に
...
音声へ時刻合わせする強制アライナー本体で、自由音素認識(ASR)機能は持たない。[§2.1](#21-認識方式の前提実歌唱データでの測定に基づく)の複合構成のうち
強制アライメント段([vocal_analysis.md §5.2.5](vocal_analysis.md#525-音素列の組み立てと単語窓の割り当て)〜[§5.2.9](vocal_analysis.md#529-segment化と結合)相当)だけを置き換える選択肢として、[`vocal_analysis.md` §5.3](vocal_analysis.md#53-sofa経路-単語単位アライメント)・[§8.3](vocal_analysis.md#83-採用ツールと代替候補)の
`sofa-forcedalign`アダプタとして選択可能である(既定は[§2.1](#21-認識方式の前提実歌唱データでの測定に基づく)の複合構成`wav2vec2-ctc-forcedalign`のまま)。
...
- **フルパイプライン比較での乖離**: 内容認識→G2P→強制アライメント→VMD生成のフルパイプラインで
  ...
  [vocal_analysis.md のS2 SOFA経路(`sofa-forcedalign`)の補足](vocal_analysis.md#s2-sofa経路sofa-forcedalignの補足)で両アダプタを選択可能なまま維持し既定を変更しない理由の根拠である。
- **チェックポイントのライセンス(既定値を持たない理由)**: 入手可能な日本語SOFAチェックポイント
  ...
  意図せずライセンス違反を犯しうるため、[vocal_analysis.md のS2 SOFA経路(`sofa-forcedalign`)の補足](vocal_analysis.md#s2-sofa経路sofa-forcedalignの補足)のとおりこの2件のいずれも既定値には
  しない(利用者保護のための方針判断)。
...
- **既知のトレードオフ**: SOFAの`force`モードは、既定構成の単語窓制約(内容認識が返す単語ごとの推定
  ...
  ([`vocal_analysis.md` §5.3](vocal_analysis.md#53-sofa経路-単語単位アライメント))。この単語境界はマージン無しの物理的な音声チャンク境界になり、
...
### 2.4 英語カタカナ化フォールバックの候補([vocal_analysis.md §5.2.4](vocal_analysis.md#524-g2p))
...
S0・S1のアダプタは1つ(採用ツールの正本は [vocal_analysis.md §8.3](vocal_analysis.md#83-採用ツールと代替候補))。S2は
...
`content_recognizer_model` 引数(既定値・任意指定。[vocal_analysis.md §5.2](vocal_analysis.md#52-wav2vec2-ctc経路-複合構成内容認識g2p強制アライメントの区間化))で選べる。
アダプタの登録と選択の扱いは [vocal_analysis.md §8.2](vocal_analysis.md#82-アダプタの登録と選択) に従う。
...
| S2 音素・母音認識(既定アダプタ) | **複合構成(内容認識 + G2P + wav2vec2 CTC 強制アライメント)を採用**。内容認識は既定値 `openai/whisper-medium`(かな限定プロンプトを渡す。単段の自由CTC認識は歌唱で不成立と実測済み。[§2](#2-s2-音素母音認識)。id・モデル・revision の固定は [vocal_analysis.md §5.2](vocal_analysis.md#52-wav2vec2-ctc経路-複合構成内容認識g2p強制アライメントの区間化)・[§8.3](vocal_analysis.md#83-採用ツールと代替候補) が正本) | transformers + pyopenjtalk-plus(in-process) | 既定値は歌唱で検証済み・高速。語彙置換による母音破綻を避けたい場合は`kana-whisper`等を任意指定できる([§2.2](#22-候補))。in-process・torch/transformersは既存と共有・ライセンス清浄 | Julius 音素認識(phone-loop構成が必要)。Allosaurusは GPL-3.0 で不可 |
| S2 音素・母音認識(選択制アダプタ) | **SOFA(単語単位アライメント)。利用者提供の専用venv・チェックポイントが必須(既定値なし。[§2.3](#23-sofa強制アライメント段の選択制代替候補))** | 利用者提供の専用venvをサブプロセス呼び出し | 単独指標だけではフルパイプラインの最終品質の優劣を判定できないため、既定アダプタと並ぶ選択肢として提供する([§2.3](#23-sofa強制アライメント段の選択制代替候補)) | ―(SOFA自体が既定アダプタへの代替候補) |

S1・S2 は [vocal_analysis.md §8.1](vocal_analysis.md#81-アダプタinterface) のアダプタinterface(Separator / Recognizer)を満たせば
...
各ツールのネイティブ出力を、[vocal_analysis.md §2](vocal_analysis.md#2-共有出力正規化中間形式) の正規化中間形式へ変換する際の要点。

| ツール | ネイティブ出力 | アダプタが取り出すもの |
|---|---|---|
| ffmpeg(自動検出) | 復号したWAV | 復号PCM(チャンネル/サンプルレート保持)のパス。レベル正規化は S0 が施す([vocal_analysis.md §3](vocal_analysis.md#3-入力読み込みs0)) |
| audio-separator(`Separator.separate`) | API が返す出力ファイルパス(Demucs v4 htdemucs_ft の分離stem) | ボーカルWAVのパス(APIの戻り値を使い、命名を推測しない) |
| 複合構成(内容認識 + G2P + CTC強制アライメント。既定値=whisper-medium・任意指定可) | 内容認識: テキスト(既定値はかな化されない場合あり、常にかなを返すモデルを任意指定することもできる) / G2P: 音素列 / アライメント: 音素ごとの開始位置(CTCスパイク) | 全時間軸被覆のセグメント列(母音/子音/gap+音素ラベル+任意の信頼度)。テキストと音素列はアダプタ内部にとどめ、共有出力に含めない。母音の終端(閉じ側)はスパイク位置からの近似で、確定は利用先(S3のRMS併用) |
| SOFA(単語単位アライメント。選択制) | HTK形式の音素ごとの開始/終了時刻(100ナノ秒単位)。AP(呼吸音)・SP(無音)ラベルを含む | 全時間軸被覆のセグメント列(母音/子音/gap+音素ラベル(IPA)+任意の信頼度)。100ナノ秒単位の時刻を秒へ変換し、Segment契約(隙間なく連続・非重複で全時間軸を被覆)を検証してから正規化する。AP・SPはgapへ写像する([vocal_analysis.md §5.3](vocal_analysis.md#53-sofa経路-単語単位アライメント)) |
...
判定(S3のRMS併用)は利用先(リップモーション生成系の入口)で行う。
...
- SOFA本体(<https://github.com/qiuqiao/SOFA>)MIT。SOFAチェックポイント(モデル重み)は本リポジトリへ
  同梱しない([§2.3](#23-sofa強制アライメント段の選択制代替候補))。同梱しないチェックポイント自体のライセンス条件の遵守は、チェックポイントを
  指定する利用者自身の選択・責任の範囲。
- ffmpeg は同梱・再配布しない([§3](#3-s0-入力読み込みffmpeg-の扱い))ため、そのビルドのライセンス(LGPL/GPL)による義務は生じない。
- Julius を採用する場合のみ、その音響モデルの個別ライセンスを確認する。
- 英語カタカナ化フォールバック([vocal_analysis.md §5.2.4](vocal_analysis.md#524-g2p))関連:
