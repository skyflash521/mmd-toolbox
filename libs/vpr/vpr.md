...
| 利用先 | 用途 | 方向 |
|---|---|---|
| 読み利用(口パク変換) | 音符の時刻・歌詞/音素(→口形イベント)・強弱(→開き量)・休符を読む | 読み |
| S-1認識測定のラベル生成 | 音符の時刻・音素(→母音/子音/休符の自動ラベル)を読む | 読み |
| 書き利用(採譜) | 音符(時刻・ピッチ・歌詞/音素・強弱)・テンポを vpr へ書き出す | 書き |

読みは口パク変換・S-1認識測定で先に必要になり、書きは採譜の着手時に必要になる(実装は読みを先行する)。
...
利用先が使う部分集合は 1.3 のとおり。読み利用とS-1認識測定は時刻・音素・(口パク変換は)強弱を使い、書き利用
