...
重い処理の進行を stderr へ1行ライブ表示する。既定は出力先が端末(TTY)のときだけ有効で、
...
停滞回避の要は「完了イベントと再描画の分離」。完了通知 update(done, total, note) は進行カウンタを更新する
だけで描画しない。実際に行を描くのは一定間隔で回るハートビート用デーモンスレッドで、update が来ない待ち
時間(重い処理の最中など)でも経過時間を進め続けるため、表示が固まって見えない。update の note は描画行へ
併記し、進捗カウンタが据え置きのまま重い処理が走る区間でも「停止ではなく処理中」だと示せる(note の
用途は呼び出し側が定める)。
...
決め方・完了行の文言・note の用途は呼び出し側に委ねる(このモジュールは他モジュールを参照しない)。

write_lock: 呼び出し側が同じ標準エラーへ別の仕組み(OSレベルのfd差し替え等)で書き込む場合に備え、構築時に
排他ロック(コンテキストマネージャプロトコルを満たすもの)を注入できる。_draw/close/summary の各書き込みは
そのロックを保持して行う。未指定時は contextlib.nullcontext() で no-op となり、ロックなしで動作する。
"""
...
class ProgressReporter:
    """段単位の進捗を1本のライブ行で表示する。stage(label) で段を開始/切り替え、
    update(done, total, note="") で進行を通知、close() で行を消して終える。summary(message) は
    完了行を1行残す。enabled 省略時は stream.isatty() で自動判定し、無効時は全メソッド no-op。
    ...
    def stage(self, label):
        """段を開始/切り替える。同じライブ行を使い、改行しない。総数・note は未確定(最初の update で確定)。
    ...
    def update(self, done, total, note=""):
        """進行を通知する(描画はしない)。総数・note は毎回更新する。段が無いときは無視する。"""
        snap = self._snap
        if snap is None:
            return
